...
Variants {
    ...
    PanelWindow {
        ...
        Item {
            ...
            // Wallpaper - Native resolution + workspace/sidebar parallax
            Item {
                ...
                StyledImage {
                    ...
                    property real fraction: {
                        ...
                        return Math.max(
                            ...
                            Math.min(
                                1,
                                (bgRoot.monitor.activeWorkspace?.id - lower) / range
                            )
                        );
                    }
                    ...
                    property real usedFractionX: {
                        ...
                        if (Config.options.background.parallax.enableWorkspace
                            && !bgRoot.verticalParallax) {
                            usedFraction = fraction;
                        }

                        if (Config.options.background.parallax.enableSidebar) {
                            let sidebarFraction =
                                bgRoot.parallaxRation
                                / bgRoot.workspaceChunkSize
                                / 2;

                            usedFraction += (
                                sidebarFraction * GlobalStates.sidebarRightOpen
                                - sidebarFraction * GlobalStates.sidebarLeftOpen
                            );
                        }
                    ...
                    // Render wallpaper at native resolution
                    sourceSize {
                        width: Math.round(
                            bgRoot.screen.width
                            * bgRoot.monitor.devicePixelRatio
                            * bgRoot.effectiveWallpaperScale
                        )

                        height: Math.round(
                            bgRoot.screen.height
                            * bgRoot.monitor.devicePixelRatio
                            * bgRoot.effectiveWallpaperScale
                        )
                    }

                    width: bgRoot.wallpaperWidth
                        / bgRoot.wallpaperToScreenRatio
                        * bgRoot.effectiveWallpaperScale

                    height: bgRoot.wallpaperHeight
                        / bgRoot.wallpaperToScreenRatio
                        * bgRoot.effectiveWallpaperScale
